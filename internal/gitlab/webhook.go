...
func (publisher *WebhookPublisher) ProcessWebhook(mrEvent *MergeRequestEvent, dispatcherEvent *dispatcher.Event, transitions *store.Transitions) error {
	...
	if actionResult := action.Execute(&ticketNumber, mrEvent, transitions); actionResult != nil {
